...
export default function Wishlist() {
  ...
  let { getWishlistProducts, deleteWishlist, numberOfWishlist } =
 ...
 async function getWishlist() {
    ...
    try {
      ...
      if (response && response.data && response.data.data) {
        ...
        setWishlistItems([]); // Reset the wishlist if the response is invalid
      }
    } catch (error) {
      console.error('Failed to fetch wishlist', error);
      setWishlistItems([]); // Reset the wishlist in case of error
    } finally {
      setLoading(false);
    }
  }
